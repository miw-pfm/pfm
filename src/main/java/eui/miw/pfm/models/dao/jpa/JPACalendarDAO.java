...
public class JPACalendarDAO extends JPATransactionGenericDAO<CalendarEntity, Integer> implements CalendarDAO{
    ...
    //Manuel Rodríguez
    @Override
    public CalendarEntity findHoliday(ProjectEntity project, Calendar holiday) {
        Query query = entityManager.createQuery("SELECT ca FROM CalendarEntity ca WHERE ca.project = ?1 AND ca.holiday = ?2");
